...
const pwRealm = "app_for_easm_realm";
...
require([
    ...
    async function populateValues() {
        ...
        try {
            ...
            await existingPw;
            if (existingPw) {
                const [workerUrl, hecUrl, hecToken, apiKey] = existingPw.properties().clear_password.split("___");
                $('#workerurl_input').val(workerUrl);
                $('#apikey_input').val(apiKey);
                $('#hectoken_input').val(hecToken);
                $('#hecurl_input').val(hecUrl);
            }
    ...
    async function completeSetup() {
        ...
        const hecTokenToSave = $('#hectoken_input').val();
        const hecUrlToSave = $('#hecurl_input').val();
        const workerUrlToSave = $('#workerurl_input').val();
        ...
        try {
            ...
            if (!existingPw) {
                ...
                passwords.create(
                    {
                        name: pwName,
                        password: workerUrlToSave + '___' + hecUrlToSave + '___' + hecTokenToSave + '___' + passwordToSave,
                        realm: pwRealm,
                    }, passwordCallback);
            } else {
